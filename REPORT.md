...
除以 `√2` 的目的是将平均符号功率归一化为 1。QPSK 的四个点均匀分布在单位圆上，相比 BPSK 频谱效率翻倍，同时在相同平均功率下仍具有较好的抗噪性能。
...
因此 I/Q 两路组合后形成 4×4 共 16 个星座点。由于原始 I/Q 取值为 `±1, ±3`，单路平均能量为 5，复符号平均功率为 10，因此需要除以 `√10` 完成归一化。16-QAM 的优点是频谱效率高，但点间距离更近，对噪声更敏感，解调复杂度也更高。
...
16-QAM 调制先将输入序列按 4 比特分组，再将前两位映射到 I 分量、后两位映射到 Q 分量，最后组合成复数符号并除以 `√10` 归一化。解调时，为提高效率，不遍历 16 个参考点，而是对 I/Q 分量分别进行门限判决：依据归一化前的取值区间恢复 2 比特格雷码，从而得到完整的 4 比特输出。
